...
description: "ROE asks what shareholders' own money earned this year. The formula, why the average equity denominator matters, and how leverage can flatter the number."
image: /assets/og/roe.png
...
**PAT** is [profit after tax]({% post_url 2026-08-22-reading-an-income-statement %}), the
bottom line of the income statement. We use the **average** of opening and closing equity (not just the closing
...
A {{ site.data.real_company.ratios.FY25.roe }}% ROE is high, even for a well-run FMCG business. It isn't
because Britannia gets more sales out of its assets: its asset turnover
(revenue ÷ average total assets) is {{ site.data.real_company.dupont.FY25.asset_turnover }}x, about the same as Desi Bites'
{{ site.data.case_study.ratios.FY25.asset_turnover }}x. The drivers are a {{ site.data.real_company.ratios.FY25.net_margin }}% net margin, a small equity
base relative to that profit, and average total assets about
{{ site.data.real_company.dupont.FY25.equity_multiplier_avg }}× average equity — so much of what Britannia owns is funded by
money that isn't shareholders' (suppliers' credit as well as loans). That's
...
- **Not checking what's driving a high ROE.** A company can boost ROE two
  ...
  these apart — that's exactly what ROCE (Return on Capital Employed), the
  subject of the next post, is for.
...
not *how* that return was achieved. A high ROE built mostly on debt carries
very different risk from one earned through efficient operations.
