...
ax.plot(stp_path.index, stp_path.values / 1e5, color=BLUE, lw=1.2, label="Same ₹12 lakh parked in the overnight fund, moved in over 12 months")
ax.plot(ov_path.index, ov_path.values / 1e5, color=GREY, lw=1.0, label="Left in the overnight fund")
...
p = pd.Series({dt: v for dt, v in path}) / 1e7
ax.plot(p.index, p.values, color=GREY, lw=1.0, label="₹1 crore in the overnight fund, ₹50,000/month from Jan 2008")
